-- =====================================================
-- Complete Fix for Shifts Table
-- =====================================================
-- Run this in your Supabase SQL Editor to completely fix the shifts table

-- 1. Create shifts table if it doesn't exist with all required columns
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='shifts' AND column_name='created_at') THEN
    ALTER TABLE shifts ADD COLUMN created_at timestamp with time zone DEFAULT now();
    RAISE NOTICE 'Added created_at column to shifts table';
  END IF;

  -- Add updated_at column
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='shifts' AND column_name='updated_at') THEN
    ALTER TABLE shifts ADD COLUMN updated_at timestamp with time zone DEFAULT now();
    RAISE NOTICE 'Added updated_at column to shifts table';
  END IF;
END $$;
...
-- 5. Create RLS policies for shifts
...
-- 6. Create volunteer_assignments table if it doesn't exist
...
-- 7. Enable RLS for volunteer_assignments
ALTER TABLE volunteer_assignments ENABLE ROW LEVEL SECURITY;

-- 8. Drop existing policies for volunteer_assignments
...
-- 9. Create RLS policies for volunteer_assignments
...
-- 12. Test the setup
SELECT 'Shifts table fixed successfully' as status;

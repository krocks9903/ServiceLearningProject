...
export default function ReportsPage() {
      ...
      const reports: EventReport[] = await Promise.all(
        (events || []).map(async (event) => {
          ...
          const uniqueVolunteers = new Set(
            assignments?.map((a) => a.volunteer_id) || []
          );
          const totalHours =
            assignments?.reduce(
  ...
  const fetchVolunteerReports = async () => {
    try {
      ...
      if (!volunteers || volunteers.length === 0) {
        console.log(
          "No volunteers found with role=volunteer, trying fallback query..."
        );
        ...
        volunteers =
          allProfiles?.filter(
            (profile) => !profile.role || profile.role === "volunteer"
          ) || [];
      ...
      const reports: VolunteerReport[] = await Promise.all(
        (volunteers || []).map(async (volunteer) => {
          ...
          const totalHours =
            hourLogs?.reduce((sum, log) => sum + (Number(log.hours) || 0), 0) ||
            0;
          ...
          const groups =
            groupMemberships
              ?.map((m: any) => m.volunteer_groups?.name)
              .filter(Boolean) || [];
      ...
      setVolunteerReports(
        reports.sort((a, b) => b.total_hours - a.total_hours)
      );
    } catch (error) {
        ...
        (groups || []).map(async (group) => {
          ...
          if (volunteerIds.length > 0) {
            ...
            hourLogs?.forEach((log) => {
              hoursByVolunteer[log.volunteer_id] =
                (hoursByVolunteer[log.volunteer_id] || 0) + Number(log.hours);
            });

            const mostActiveId = Object.entries(hoursByVolunteer).sort(
              (a, b) => b[1] - a[1]
            )[0]?.[0];
            ...
            return {
              ...
              total_hours: totalHours,
              average_hours_per_volunteer:
                volunteerIds.length > 0
                  ? Math.round(totalHours / volunteerIds.length)
                  : 0,
              most_active_volunteer:
                volunteerIds.length > 0 ? mostActiveName : "N/A",
            };
          }
  ...
  const exportToCSV = () => {
    ...
    switch (activeTab) {
      ...
      case "groups":
        csvContent =
          "Group,Members,Total Hours,Avg Hours/Volunteer,Most Active\n";
                ...
                <tbody>
                  {groupReports.map((report) => (
                    <tr key={report.group_id}>
                      ...
                      <td style={styles.tableCell}>{report.total_hours}</td>
                      <td style={styles.tableCell}>
                        {report.average_hours_per_volunteer}
                      </td>
                      <td style={styles.tableCell}>
                        {report.most_active_volunteer}
                      </td>
                    </tr>
                  ))}
                </tbody>

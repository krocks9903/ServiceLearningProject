...
export default function EventsPage() {
    ...
    try {
      ...
      if (!user) {
        ...
        const publicEvents = (allEvents || []).filter(
          (e) => !(e as Event & { is_private?: boolean }).is_private
        );
      ...
      const { data: visibilityData } = await supabase
        ...
        .select("event_id")
        .or(
          `volunteer_id.eq.${user.id},group_id.in.(SELECT group_id FROM volunteer_group_memberships WHERE volunteer_id.eq.${user.id})`
        );

      const visiblePrivateEventIds = new Set(
        (visibilityData || []).map((v) => v.event_id)
      );

      // Filter events: show public events OR private events the user has access to
      const filteredEvents = (allEvents || []).filter(
        (e) =>
          !(e as Event & { is_private?: boolean }).is_private ||
          visiblePrivateEventIds.has(e.id)
      );
  ...
  return (
    ...
    >
      ...
      <div
        style={{ padding: "0 2rem 2rem", maxWidth: "1200px", margin: "0 auto" }}
      >
        {/* Calendar Toggle */}
        <div
          style={{

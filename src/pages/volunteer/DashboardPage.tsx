...
export default function DashboardPage() {
  ...
  const handleAddHours = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        showNotification(
          "error",
          `Failed to add hours: ${error.message || "Please try again."}`
        );
        return;
      }
      ...
      showNotification(
        "success",
        "Hours submitted successfully! Awaiting verification."
      );
    } catch (error) {
  ...
  const progressPercentage = Math.min(
    (dashboardData.totalHours / dashboardData.hoursTarget) * 100,
    100
  );
  const monthProgressPercentage = Math.min(
    (dashboardData.hoursThisMonth / 20) * 100,
    100
  );
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Pending Hours Info */}
      {dashboardData.recentHours.filter((log) => log.status === "Pending")
        .length > 0 && (
        <div style={styles.pendingHoursInfo}>
          ...
          <div>
            <div style={styles.pendingHoursTitle}>
              {
                dashboardData.recentHours.filter(
                  (log) => log.status === "Pending"
                ).length
              }{" "}
              hour log(s) awaiting approval
            </div>
            <div style={styles.pendingHoursText}>
              Your submitted hours are being reviewed by an administrator. Once
              approved, they'll be added to your total.
            </div>
          </div>
        </div>
      )}
      ...
      >
        <div style={{ flex: 1, minWidth: "250px" }}>
          ...
          >
            Check in when you arrive at the volunteer location using your
            volunteer number or email
          </p>
        </div>
          ...
          <div style={styles.cardBody}>
            ...
            <div style={styles.progressInfo}>
              ...
              {progressPercentage < 100 && (
                <span>
                  {Math.round(
                    dashboardData.hoursTarget - dashboardData.totalHours
                  )}{" "}
                  hours remaining
                </span>
              )}
                  ...
                  return (
                    <div key={event.id} style={styles.eventItem}>
                      <div style={styles.eventDateBadge}>
                        <div style={styles.eventDay}>{eventDate.getDate()}</div>
                    ...
                    <div style={styles.hoursRight}>
                      ...
                      <div
                        style={{
                          ...styles.statusBadge,
                          backgroundColor:
                            log.status === "Approved" ? "#d1fae5" : "#fef3c7",
                          color:
                            log.status === "Approved" ? "#065f46" : "#92400e",
                        }}
                      >
                        {log.status}
                      </div>
            ...
            <div style={styles.modalBody}>
              <div style={styles.formGroup}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewHoursData({ ...newHoursData, hours: e.target.value })
                  }
                  placeholder="0"
                  step="0.5"
                  min="0"
